...
`FitnessApp.slnx` is the only application solution. `FitnessApp.Server` is the only hosted startup project, launched with `dotnet run --project src/FitnessApp.Server --launch-profile https`; it hosts both the Blazor WebAssembly client and ASP.NET Core API. Strength-program management and running are implemented within these existing layers and projects, with no separate fitness-feature solution, application, executable, host, or startup process.
