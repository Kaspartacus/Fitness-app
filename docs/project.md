...
- The private `/kalender` route is the shared calendar: it presents active planned running sessions, actual manual and retired-plan running results, current/future strength-schedule occurrences, and actual completed strength snapshots. Running results render on their actual `DateOnly` date and appear only as a non-counting relationship on a different planned date. Calendar-started strength completions retain their original scheduled occurrence (including a move) so that exact occurrence is no longer projected as planned; legacy and standalone strength history remains unlinked. Strength has only a weekly schedule model, so past schedule occurrences are never manufactured; completed strength history uses the persisted Copenhagen-local completion day. Calendar reads are owner-scoped, inclusive, and bounded to 93 days.
- Owner-scoped Profile/Settings pages for approved users, including persisted display name, optional height/weight and nutrition targets, fixed app units, security/password-reset guidance, and the existing secure logout flow. The Figma-matched Person control opens Settings from Home, Strength, Running, and Calendar.
- Settings includes a server-policy-protected Admin-only link and pending count for the existing registration-approval flow; it does not duplicate approval or rejection behavior.
- Persisted in-app notifications have owner-scoped read state and target navigation. New pending registrations create one idempotent generic notice per opted-in approved administrator; opted-in users receive idempotent Copenhagen-local reminders for today's planned running or strength activity when Settings notifications are read. Browser push and email delivery are intentionally unavailable in this slice.
- Garmin is an isolated, persistent demo/mock connection state only: it requests no Garmin credentials and never synchronizes, imports, or fabricates activities, statistics, or calendar entries.
...
- Profile contains settings and logout. Profile subpages do not repeat logout. Profile preferences are owner-scoped; in-app notifications use durable unique source keys, server-derived Copenhagen-local dates, and do not imply delivery while the app is closed.
