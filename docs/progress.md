...
- The translated Danish flow is implemented at `/loeb` (overview), `/loeb/ny` (seven-step setup), `/loeb/plan` (week selection), `/loeb/plan/{sessionId}` (session detail), `/loeb/plan/{sessionId}/aktiv` (active run), `/loeb/registrer` and `/loeb/plan/{sessionId}/registrer` (manual/planned registration), `/loeb/resultater` (result history), `/loeb/resultater/{resultId}` plus `/ret` (result detail/correction), and `/kalender` (month/day selection). An empty overview leads to setup; setup creates or replaces a personal plan; plan/calendar sessions open their details; a selected session can start the active timer or be completed/logged; manual registration creates a result; result detail permits correction; the separate result history reopens all manual and archived-plan results; calendar selection opens the relevant scheduled session or saved result. The overview intentionally omits **Seneste løb**, which is absent from the current reference.
- Existing shared visual patterns are reused: charcoal cards and navy/light-blue states, top-left back controls, the shared centered modal for confirmations/cancellation guards, and the mobile-only bottom navigation. The Figma prototype did not define all loading, empty, validation, retry, cancellation, conflict, or error states, so those are deliberate product extensions rather than reproduced prototype behavior.
